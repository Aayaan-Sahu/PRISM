...
def _normalize_landmarks(preds):
    """Return one face landmark tensor in a stable shape: (1, 68, 2)."""
    if preds is None:
        return None
    try:
    ...
    # face_alignment may return (num_faces, 68, 2) or (68, 2).
...
def _get_device():
    ...
    NOTE: MPS (Apple Silicon GPU) is intentionally excluded because
    Dolphin uses several ops that MPS doesn't support yet
    (adaptive_avg_pool1d with non-divisible sizes, .type() patterns, etc).
    CPU is slower but guaranteed to work.
    """
...
def detectface(video_input_path, output_path, detect_every_N_frame, scalar_face_detection, number_of_speakers):
    ...
    for i, frame in enumerate(frames):
        ...
        if i == 0:
            ...
            for j in range(number_of_speakers):
                ...
                preds = _normalize_landmarks(fa.get_landmarks(np.array(face)))
    ...
    for i in range(number_of_speakers):    
        ...
        # Save face frames as numpy array (avoids lossy mp4 encode/decode
        # cycle that can produce frame count mismatches across ffmpeg versions)
        face_frames = [np.array(frame) for frame in faces_dic[i]]
        if face_frames:
            face_npy_path = os.path.join(output_path, 'faces', 'speaker' + str(i+1) + '.npy')
            np.save(face_npy_path, np.array(face_frames))
...
def crop_patch(mean_face_landmarks, video_frames, landmarks, window_margin, start_idx, stop_idx, crop_height, crop_width, STD_SIZE=(256, 256)):

    """Crop mouth patch
    :param np.ndarray video_frames: array of BGR face frames (N, H, W, 3)
    ...
    # Convert RGB frames to BGR if needed (npy saves are RGB from PIL)
...
def crop_mouth(video_direc, landmark_direc, filename_path, save_direc, convert_gray=False, testset_only=False):
    ...
    for filename_idx, line in enumerate(lines):
        ...
        # Load face frames from .npy (lossless, no ffmpeg decode needed)
        face_npy_path = os.path.join(video_direc, filename+'.npy')
        landmarks_pathname = os.path.join(landmark_direc, filename+'.npz')
        dst_pathname = os.path.join( save_direc, filename+'.npz')

        face_frames = np.load(face_npy_path)
        print(f'  Loaded {len(face_frames)} face frames from {face_npy_path}')
        ...
        # Ensure face frames and landmarks have the same count
        if len(face_frames) != len(preprocessed_landmarks):
            min_len = min(len(face_frames), len(preprocessed_landmarks))
            print(f'  Warning: frame/landmark mismatch ({len(face_frames)} vs {len(preprocessed_landmarks)}), truncating to {min_len}')
            face_frames = face_frames[:min_len]
            preprocessed_landmarks = preprocessed_landmarks[:min_len]

        # -- crop
        _SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
        mean_face_landmarks = np.load(os.path.join(_SCRIPT_DIR, 'assets', '20words_mean_face.npy'))
        sequence = crop_patch(mean_face_landmarks, face_frames, preprocessed_landmarks, 12, 48, 68, 96, 96)
...
def process_video(input_file, output_path, number_of_speakers=2, 
    ...
    # Merge video with separated audio for each speaker (only if not skipped)
    output_files = []
    for i in range(number_of_speakers):
        audio_input = os.path.join(output_path, f"speaker{i+1}_est.wav")
        if skip_video_rendering:
            # We skip the heavy video encoding because the application only
            # requires the separated .wav for enrollment
            output_files.append(audio_input)
        else:
