"""
Modal Cloud TSE Separator (WeSep)
=================================

Deploys reference-conditioned target speech extraction to Modal GPU and exposes
an in-container FastAPI websocket server for low-latency streaming.

Deploy:
    modal deploy modal_tse_separator.py

Local dev:
    modal serve modal_tse_separator.py
"""
...
FASTAPI_APP_CODE = r'''
"""In-container FastAPI app for WeSep real-time TSE."""
...
def _fit_to_length(wav: torch.Tensor, length: int) -> torch.Tensor:
    if wav.ndim == 1:
        wav = wav.unsqueeze(0)
    if wav.shape[0] > 1:
        wav = wav[:1]
...
def _decode_ref_pcm16_b64(b64_data: str, expected_sr: int) -> torch.Tensor:
...
def _compute_embedding(wav_cpu: torch.Tensor) -> torch.Tensor:
    wav_dev = wav_cpu.to(device).float()
    with torch.no_grad():
        emb = spk_model.encode_batch(wav_dev).squeeze()
    if emb.ndim == 0:
        emb = emb.unsqueeze(0)
    norm = emb.norm(p=2)
...
def _similarity(pred_full_cpu: torch.Tensor, ref_emb: torch.Tensor) -> float:
    if float(pred_full_cpu.abs().max().item()) < 1e-6:
        return 0.0
    if float(ref_emb.abs().max().item()) < 1e-8:
        return 0.0
    pred_dev = pred_full_cpu.to(device).float()
    with torch.no_grad():
        pred_emb = spk_model.encode_batch(pred_dev).squeeze()
    if pred_emb.ndim == 0:
...
def _extract_for_target(mix_context_cpu: torch.Tensor, ref_wav_cpu: torch.Tensor) -> torch.Tensor:
    pred = extractor.extract_speech_from_pcm(mix_context_cpu, MODEL_SR, ref_wav_cpu, MODEL_SR)
    if pred is None:
    ...
    if pred.shape[0] > 1:
        pred = pred[:1]
    pred = _fit_to_length(pred, mix_context_cpu.shape[1]).float().cpu()
    return pred


def _parse_targets(init_msg: dict) -> list[dict]:
    ...
    for i, entry in enumerate(entries[:MAX_TARGETS]):
        ...
        try:
            ...
            ref_wav = _decode_ref_pcm16_b64(ref_b64, ref_sr)
            ref_emb = _compute_embedding(ref_wav)
            parsed.append(
                {
                    "id": target_id,
                    "ref_wav": ref_wav,
...
def _load_models_once():
    global extractor, spk_model, device, load_error
    try:
        _ensure_paths()
        ...
        extractor.set_device(str(device))
        ...
        print(f"MODEL LOAD FAILED:\n{load_error}", file=sys.stderr, flush=True)


_load_models_once()

web_app = FastAPI(title="Spatial Audio TSE (WeSep)")


@web_app.get("/health")
async def health():
    return {
        "status": "ok" if extractor is not None and spk_model is not None else "models_not_loaded",
...
async def ws_endpoint(websocket: WebSocket):
    ...
    if extractor is None or spk_model is None:
        await websocket.send_text(json.dumps({"type": "error", "detail": f"Models not loaded: {load_error}"}))
    ...
    try:
        ...
        targets = _parse_targets(init_msg)
        ...
        while True:
            ...
            if "text" in msg and msg["text"]:
                ...
                try:
                    ...
                    if reload_msg.get("type") == "init_tse_v1":
                        targets = _parse_targets(reload_msg)
                        await websocket.send_text(
                            json.dumps(
                                {
                                    "type": "ready",
            ...
            # WeSep extractor API currently exposes single-reference extraction.
            # We bound target count and use sequential fallback per target.
            ...
            for t in targets:
                try:
                    pred_full = _extract_for_target(context_buf, t["ref_wav"])
                    sim = _similarity(pred_full, t["ref_emb"])
...
def serve():
    """Write the FastAPI app and launch uvicorn inside the Modal container."""
    import subprocess
...
def main():
    ...
    print("  deploy: modal deploy modal_tse_separator.py")
    print("  dev:    modal serve  modal_tse_separator.py")
