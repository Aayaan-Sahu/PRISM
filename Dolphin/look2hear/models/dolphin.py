...
def _adaptive_avg_pool1d_mps_safe(x: torch.Tensor, output_size):
    out_size = output_size[-1] if isinstance(output_size, (tuple, list)) else int(output_size)
    if x.device.type == "mps" and out_size > 0 and (x.shape[-1] % out_size != 0):
        pooled_cpu = F.adaptive_avg_pool1d(x.cpu(), output_size)
        return pooled_cpu.to(device=x.device, dtype=x.dtype)
...
class DU_MHSA(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos_k: torch.Tensor):
        ...
        x_down = _adaptive_avg_pool1d_mps_safe(x, down_len)
        x = x.permute([0, 2, 1])
...
class UConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for fea in output:
            global_f = global_f + _adaptive_avg_pool1d_mps_safe(fea, output[-1].shape[-1])
...
class Separator(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        for idx in range(self.num_stages):
            ...
            skip.append(skip_)
            fusion_x = fusion_x + _adaptive_avg_pool1d_mps_safe(x, min_len)
    ...
    def pad_signal(self, input: torch.Tensor):
        ...
        if rest > 0:
            pad = torch.zeros(batch_size, ndim, rest, device=input.device, dtype=input.dtype)
